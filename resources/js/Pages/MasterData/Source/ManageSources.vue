    ...
    <div class="p-4">
      <div class="container mx-auto p-2">
        ...
        <div class="flex justify-between items-center mb-6">
          <h1 class="text-2xl font-semibold text-gray-800">Kelola Source</h1>
          <PrimaryButton @click="saveSelected">Simpan Source Dipilih</PrimaryButton>
        </div>
          ...
          <table class="min-w-full">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Nama Source</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-200">
              ...
              <template v-else>
                <tr>
                  <td colspan="2" class="px-6 py-4 text-center text-sm text-gray-500">Tidak ada data source.</td>
                </tr>
              </template>
            </tbody>
          </table>
...
// Ambil semua subkategori berdasarkan source_id
const getAllSubCategories = (category_id) => {
  return props.subCategories.filter(sub => sub.source_id === category_id);
};
...
const onSubCategorySelect = (subCategory) => {
  if (selectedSubCategories.value.includes(subCategory.id)) {
    if (!selectedCategories.value.includes(subCategory.source_id)) {
      selectedCategories.value.push(subCategory.source_id);
    }
  } else {
    const hasOtherSelectedSubCategories = props.subCategories.some(
      sub => sub.source_id === subCategory.source_id && selectedSubCategories.value.includes(sub.id)
    );
    if (!hasOtherSelectedSubCategories) {
      selectedCategories.value = selectedCategories.value.filter(id => id !== subCategory.source_id);
    }
  }
};
...
const saveSelected = () => {
  router.post(route('sources.save'), {
